...
url = "http://www.sefton.gov.uk/your-council/open-data,-transparency-and-foi/open-data-(transparency).aspx"
errors = 0
# Set up functions
...
for block in blocks:
    ...
    for link in links:
        if '.csv' in link['href']:
            ...
            csvfile = url.split('spend')
            csvf = csvfile[-1]
            csvcsv = csvf.split('.csv')[0].strip()[-4:]
            csvYr = csvcsv
            csvmnth = csvf.split('.csv')[0].strip().split('_')[-1]
            csvMth = csvmnth.replace('-', '')
            if len(csvcsv) == 0:
                csvYr = '2014'
                csvMth = 'april'
            csvMth  = csvMth[:3]
